...
function RainyDay(canvasid, sourceid, width, height, opacity, blur) {
  ...
  this.prepareBackground(blur || 20, width, height);
  this.w = this.canvas.width;
  this.h = this.canvas.height;

  // create the glass canvas
  this.prepareGlass(opacity || 1);
  ...
  if (speed > 0) {
    ...
    if (this.VARIABLE_COLLISIONS) {
      ...
      if (maxDropRadius > 0) {
        ...
        const mwi = Math.ceil(this.w / maxDropRadius);
        const mhi = Math.ceil(this.h / maxDropRadius);
...
Drop.prototype.animate = function () {
  this.intid = setInterval(
    (function (self) {
      return function () {
        const stopped = self.rainyday.gravity(self);
        if (!stopped && self.rainyday.trail) {
          self.rainyday.trail(self);
        }
        if (self.rainyday.VARIABLE_COLLISIONS) {
          const collision = self.rainyday.matrix.update(self, stopped);
          if (collision) {
